    ...
    >
      {youtubeThumb ? (
        ...
        <div
          ...
          style={{ height: compact ? 44 : 60, background: "color-mix(in srgb, var(--accent) 8%, var(--active-row))" }}
        >
          <svg width={compact ? "18" : "22"} height={compact ? "18" : "22"} fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24" style={{ color: "var(--accent)" }}>
      ...
      <div className={compact ? "px-2.5 py-2 relative" : "px-3 py-2.5 relative"}>
        ...
        >
          {data.sourceType === "youtube" ? "YouTube" : data.sourceType === "note" ? "Note" : "PDF"}
        </p>
        {data.onDelete && (
          <button
